...
class DDDQN(DQN):
    ...
    def q_values(self):
        ...
        self.h_fc1 = W_apa = self.weight_variable(
            [1568, self.action_space], method="xavier")
        ...
        # 1/|A| * Sum_(a') A(s,a';theta,alpha) ).
        predicted_q_values = fc_sv + \
            tf.subtract(fc_apa, tf.reduce_mean(fc_apa, axis=1, keep_dims=True))
